...
<script>
	...
	function showPopup2() {
		window.open("UserReport.do?target=${bet.betOwner}&userId=${userId}",
				"a", "width=500, height=300, left=100, top=50");
	}
</script>
</head>
		...
		<div id="container">
			...
			<div id="main" class="clearfix">
				<div id="page">
					...
					<c:if test="${bet.state eq '대기' }">
						<c:if
							test="${userId eq bet.betOwner or userId eq teamA.leader.userId}">
							<form action="ImageA.do" method="POST"
								enctype="multipart/form-data">
								<input type="hidden" name="betId" value="${bet.betId }">
								<table>
									<tr>
										<td><input type="file" name="photoA">
											<button type="submit" value="올리기">A팀 사진올리기</button></td>
										<td></td>
										<td></td>
										<td></td>
										<td></td>
										...
										<c:if
											test="${userId eq bet.betOwner or userId eq teamB.leader.userId}">
											<form action="ImageB.do" method="POST"
												enctype="multipart/form-data">
												<input type="hidden" name="betId" value="${bet.betId }">
												<td align="right"><input type="file" name="photoB">
													<button type="submit" value="올리기">B팀 사진올리기</button></td>
									</tr>
									</form>
									</c:if>
								</table>
						...
						<script type="text/javascript">
							$('.btn4').click(
									function() {
										if ($(":radio[name=" + $(vote).attr("name")	+ "]:checked").length == 0) {
											alert("투표를 해주세요");
										}else
						...
						<form action="betVote.do" method="post" name="sub2">
							<input type="hidden" name="betId" value="${bet.betId }">
							<table>
								<tr>
									<td><img class="imgs" src="/images/${bet.photoA }"alt="이미지를 올려주세요" /> <br> <br> <c:if
									...
									<td><img class="imgs" src="/images/${bet.photoB }" alt="이미지를 올려주세요" /> <br> <br> <c:if
											test="${bet.state eq '진행'}">
											<c:if
												test="${userId eq teamB.leader.userId or userId eq teamA.leader.userId}">
							...
							<c:if test="${bet.state eq '진행'}">
								<c:if
									test="${userId eq teamB.leader.userId or userId eq teamA.leader.userId}">
									<div align="center">
										<button type="submit" class="btn btn btn-warning btn4">투표하기</button>
									</div>
								</c:if>
							</c:if>
						</form>
               ...
               <c:forEach var="p" items="${teamB.players }">
                  ...
                  <c:if
								...
								<a
									href="deleteplayerByTeam.do?betId=${bet.betId }&userId=${p.userId }&teamId=${teamA.teamId}">[X]</a>
							</c:if>,                  
               </c:forEach>
						...
						<div class="panel-footer">
							<div class="write_area">
								<form action="registComment.do" method="POST">
									...
									<br> <input type="submit" class="comment_submit"
										value="작성">
								</form>
							</div>
						</div>
