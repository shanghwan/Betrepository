...
<body>
	<div id="wrapper">
		<div id="container">
			...
			<div id="banner">
				<h1 class="page-title">
					BetOf${bet.betWay }(${bet.betId })
					<div align="center">${bet.title }</div>
				</h1>
			</div>
			<!-- // end #banner -->
			...
			<c:if test="${userId eq 'admin' }">
				<a href="deleteBetAllReport.do?target=${bet.betId }"><button
						type="button" class="btn btn btn-warning">내기삭제</button></a>
			</c:if>
				...
				<c:if test="${userId eq list }">
					<div align="left">
						<form action="gameJoin.do" method="post">
							...
							<input type="text" name="pointBet" placeholder="포인트 입력 "
								size="12"></input> <input type="radio" name="teamName" value="A">Team
							A <input type="radio" name="teamName" value="B">Team B
